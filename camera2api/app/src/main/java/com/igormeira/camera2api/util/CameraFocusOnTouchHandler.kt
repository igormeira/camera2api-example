package com.igormeira.camera2api.util
...
class CameraFocusOnTouchHandler(
    ...
    override fun onTouch(view: View, motionEvent: MotionEvent): Boolean {
        ...
        val y =
            (motionEvent.x / view.width.toFloat() * sensorArraySize!!.height()
                .toFloat()).toInt()
        val x =
            (motionEvent.y / view.height.toFloat() * sensorArraySize.width()
                .toFloat()).toInt()
        val halfTouchWidth =
        ...
        if (isMeteringAreaAFSupported) {
            mPreviewRequestBuilder.set(
                CaptureRequest.CONTROL_AF_REGIONS, arrayOf(focusAreaTouch)
            )
        }
        mPreviewRequestBuilder.set(
    ...
    private val isMeteringAreaAFSupported: Boolean
        private get() {
            val value = mCameraCharacteristics.get(
